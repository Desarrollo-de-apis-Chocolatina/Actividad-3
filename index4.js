...
// ==========================
// NUEVAS ESTRUCTURAS - ACTIVIDAD 4
// ==========================
...
// ==========================
// ENDPOINTS DE REPORTES - ACTIVIDAD 4
// ==========================

// GET /reportes/tareas-por-asignado
app.get('/reportes/tareas-por-asignado', (req, res) => {
    const conteo = {};
    for (const tarea of tareas) {
        const nombreAsignado = getAsignadoNombre(tarea);
        if (nombreAsignado) {
            conteo[nombreAsignado] = (conteo[nombreAsignado] || 0) + 1;
        }
    }
    
    const resultado = Object.keys(conteo).map(nombre => ({
        asignado: nombre,
        totalTareas: conteo[nombre]
    }));
    
    res.json(resultado);
});

// GET /reportes/distribucion-pesos
app.get('/reportes/distribucion-pesos', (req, res) => {
    const conteo = {};
    for (const tarea of tareas) {
        const peso = tarea.peso;
        conteo[peso] = (conteo[peso] || 0) + 1;
    }
    
    const resultado = Object.keys(conteo).map(peso => ({
        peso: Number(peso),
        totalTareas: conteo[peso]
    }));
...
// GET /reportes/completadas
app.get('/reportes/completadas', (req, res) => {
    const { desde, hasta } = req.query;
    
    if (!desde || !hasta) {
        return res.status(400).json({ mensaje: "Los parámetros 'desde' y 'hasta' son requeridos." });
    }
    ...
    if (isNaN(fechaDesde.getTime()) || isNaN(fechaHasta.getTime())) {
        return res.status(400).json({ mensaje: "Las fechas 'desde' o 'hasta' no son válidas." });
    }

    const completadasEnRango = historialCambios.filter(cambio => {
        if (cambio.tipo !== 'estado' || cambio.a !== 'completada') return false;
        
        const fechaCambio = new Date(cambio.fecha);
        return fechaCambio >= fechaDesde && fechaCambio <= fechaHasta;
    });
    
    const tareasCompletadas = completadasEnRango.map(cambio => {
        const tarea = tareas.find(t => t.id === cambio.tareaId);
        return {
            tareaId: cambio.tareaId,
            fechaCompletada: formatFechaHora(cambio.fecha),
            tarea: tarea || "Tarea eliminada"
        };
    });

    res.json({
        totalCompletadas: completadasEnRango.length,
        desde: formatFechaHora(fechaDesde),
        hasta: formatFechaHora(fechaHasta),
        detalle: tareasCompletadas
    });
});

// ==========================
// ENDPOINTS DE ETIQUETAS (TAGS) - ACTIVIDAD 4
// ==========================
...
app.post('/etiquetas', (req, res) => {
    ...
    if (!nombre || typeof nombre !== 'string' || nombre.trim() === '') {
        return res.status(400).json({ mensaje: "El nombre de la etiqueta es requerido" });
    }
    
    const nombreNormalizado = nombre.trim();
    
    const existente = etiquetas.find(e => normalizeText(e.nombre) === normalizeText(nombreNormalizado));
    if (existente) {
        return res.status(400).json({ mensaje: "La etiqueta ya existe", etiqueta: existente });
    }
...
// POST /tareas/:id/etiquetas/:tagId
app.post('/tareas/:id/etiquetas/:tagId', (req, res) => {
    ...
    const etiqueta = etiquetas.find(e => e.id === tagId);
    if (!etiqueta) return res.status(404).json({ mensaje: "Etiqueta no encontrada" });
    
    const existeAsociacion = tareaEtiquetas.find(te => te.tareaId === tareaId && te.etiquetaId === tagId);
    if (existeAsociacion) {
        return res.status(400).json({ mensaje: "La etiqueta ya está asociada a esta tarea" });
    }
    
    tareaEtiquetas.push({ tareaId, etiquetaId: tagId });
    
    res.status(201).json({ mensaje: "Etiqueta asociada correctamente", tareaId, tagId });
});

// DELETE /tareas/:id/etiquetas/:tagId
app.delete('/tareas/:id/etiquetas/:tagId', (req, res) => {
    const tareaId = Number(req.params.id);
    const tagId = Number(req.params.tagId);
    
    const index = tareaEtiquetas.findIndex(te => te.tareaId === tareaId && te.etiquetaId === tagId);
    
    if (index === -1) {
        return res.status(404).json({ mensaje: "La asociación no existe" });
    }
    
    tareaEtiquetas.splice(index, 1);
    
    res.status(204).send();
});

// GET /tareas/:id/etiquetas
app.get('/tareas/:id/etiquetas', (req, res) => {
    const tareaId = Number(req.params.id);
    
    const tarea = tareas.find(t => t.id === tareaId);
    if (!tarea) return res.status(404).json({ mensaje: "Tarea no encontrada" });
    
    const asociaciones = tareaEtiquetas.filter(te => te.tareaId === tareaId);
